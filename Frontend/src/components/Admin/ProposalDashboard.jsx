...
export default function ProposalsDashboard() {
    ...
    // Reset to first page whenever search, filters, sorting, or page size change
    useEffect(() => {
        setCurrentPage(1);
    ...
    const [showUpdateRequestDialog, setShowUpdateRequestDialog] =
        useState(false);
        ...
        try {
            ...
            toast.error(
                error.response?.data?.message ||
                    "Failed to delete update request",
            );
        }
    };
    ...
    const getUpdateRequestStatus = (proposalId) => {
        const updateRequest = updateRequests.find(
            (req) =>
                req.proposal_id._id === proposalId ||
                req.proposal_id === proposalId,
        );
    ...
    const openReviewerDetailsDialog = (proposal) => {
        ...
        const proposalAssignments = reviewAssignments.filter((assignment) => {
            ...
            const assignmentProposalIdStr = (
                typeof assignment.proposal_id === "object" &&
    ...
    const fetchReviewerAssignments = async () => {
        try {
            const response = await api.get(
                "/api/admin/reviewer/review-details",
            );
        ...
        try {
            ...
            if (response.data) {
                        ...
                        reviewers: Array.isArray(p.reviewer)
                            ? p.reviewer.map((r) => {
                                  ...
                                  return {
                                      ...
                                      markSheetUrl:
                                          assignment?.mark_sheet_url || null,
                                      evaluationSheetUrl:
                ...
                const stats = {
                    ...
                    pending: formattedProposals.filter((p) => p.status === 0)
                        .length,
                    pending_review: formattedProposals.filter(
                        (p) => p.status === 1,
                    ).length,
                    reviewed: formattedProposals.filter((p) => p.status === 2)
                        .length,
                    allocated: formattedProposals.filter((p) => p.status === 3)
                        .length,
                };
        ...
        try {
            ...
            if (response.data) {
                ...
                if (newStatus === 3 && allocatedBudget) {
                    await api.post(
                        "/api/admin/research-proposal/approval-budget",
                        {
    ...
    const confirmAllocation = () => {
        try {
            api.post("/api/admin/research-proposal/approval-budget", {
            ...
            toast.error(
                error.response?.data?.message || "Failed to allocate budget",
            );
        }
    };
    ...
    const assignReviewer1 = async (proposal) => {
        ...
        if (
            ...
            toast.error(
                "Please enter a valid expiration period (minimum 1 day)",
            );
            return;
        }
        ...
        try {
            ...
            if (existingReviewer1) {
                ...
                // Create new reviewer
                const newReviewer1Res = await api.post(
                    "/api/admin/reviewer/add",
                    {
                        name: reviewerName1,
    ...
    const assignReviewer2 = async (proposal) => {
        ...
        try {
            ...
            if (existingReviewer2) {
                ...
                // Create new reviewer
                const newReviewer2Res = await api.post(
                    "/api/admin/reviewer/add",
                    {
                        name: reviewerName2,
    ...
    const filteredProposals = proposals
        ...
        .sort((a, b) => {
            ...
            if (key === "cgpa") {
                ...
                const aCgpa =
                    a.applicantType === "Student" ? Number(a.cgpa) || 0 : 0;
                const bCgpa =
                    b.applicantType === "Student" ? Number(b.cgpa) || 0 : 0;
    ...
    const removeReviewer = async (proposal, reviewer) => {
        ...
        try {
            ...
            await api.post("/api/admin/research-proposal/reviewer/remove", {
                ...
                proposal_type:
                    proposalForReviewerDelete.applicantType.toLowerCase(),
            });
            ...
            if (
                ...
                selectedProposalReviewers.proposal.id ===
                    proposalForReviewerDelete.id
            ) {
                setSelectedProposalReviewers({
                    ...selectedProposalReviewers,
            ...
            toast.error(
                error.response?.data?.message || "Failed to remove reviewer",
            );
        } finally {
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="w-full sm:w-60">
                        ...
                        >
                            <SelectTrigger className="w-full border-emerald-200 dark:border-emerald-800">
                                <CalendarClock className="h-4 w-4 mr-2 text-emerald-600 dark:text-emerald-400" />
            ...
            <div className="border rounded-md">
                                                ...
                                                {proposal.status !== 3 &&
                                                    proposal.status === 2 && (
                                                        <Button
                                                            ...
                                                            size="icon"
                                                            onClick={() => {
                                                                setSelectedProposalForAllocation(
                                                                    proposal,
                                                                );
                                                                setAllocatedAmount(
                ...
                {/* Pagination Bar */}
                {totalItems > 0 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 border-t bg-slate-50/50 dark:bg-slate-900/20 text-sm">
                        <div className="text-muted-foreground text-xs sm:text-sm">
                            ...
                            {totalItems !== proposals.length && (
                                <span className="text-xs text-muted-foreground ml-1">
                                    (filtered from {proposals.length} total)
                                </span>
                            )}
                        </div>
            ...
            >
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Set the allocated budget for this proposal. This
                            will also mark the proposal as Allocated.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-3">
                                ...
                                <div className="text-sm">
                                    ...
                                    <p className="font-medium">
                                        BDT{" "}
                                        {
                                            selectedProposalForAllocation?.totalBudget
                                        }
                                    </p>
                                </div>
                                <div className="text-sm">
            ...
            >
                <DialogContent>
                    ...
                    <div className="space-y-4 py-3">
                        ...
                        {selectedProposalReviewers?.reviewers.map(
                            (reviewer, index) => (
                                <div
                                    key={index}
            ...
            >
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Are you sure you want to remove this reviewer from
                            the proposal?
                        </DialogDescription>
                    </DialogHeader>
                                ...
                                <DialogFooter className="mt-4">
                                    <Button
                                        ...
                                        onClick={() => {
                                            ...
                                            setShowUpdateRequestStatusDialog(
                                                false,
                                            );
                                        }}
                                    >
