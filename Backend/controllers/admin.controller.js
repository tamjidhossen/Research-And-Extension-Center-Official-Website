...
const updateRequestStatus = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `Request ${status} successfully`, token: token });
  } catch (error) {
...
const deleteReviewer = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Reviewer deleted successfully" });
  } catch (error) {
...
const updateApprovalBudget = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Approval budget updated successfully", Proposal });
  } catch (error) {
  // console.error(error);
    return res
      .status(500)
...
const searchReviewerByFiscalYear = async (reviewer_id, fiscal_year) => {
  try {
    ...
    const studentProposals = await StudentProposal.find({ fiscal_year }).select(
      "_id"
    );
    const teacherProposals = await TeacherProposal.find({ fiscal_year }).select(
      "_id"
    );
