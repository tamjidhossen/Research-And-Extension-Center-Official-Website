    ...
    try {
        ...
        if (!firstDocument || firstDocument.fiscal_year === "None") {
            ...
            return res
                ...
                .json({
                    success: false,
                    message: "Application process not started",
                });
        }
        const {
            project_director,
        ...
        const proposal = new TeacherProposal({
            ...
            total_budget,
            pdf_url_part_A:
                req.files && req.files["partA"] && req.files["partA"][0]
                    ? req.files["partA"][0].path
                    : null,
            pdf_url_part_B:
                req.files && req.files["partB"] && req.files["partB"][0]
                    ? req.files["partB"][0].path
                    : null,
        });

        await proposal.save();
        res.status(201).json({
            message: "Teacher proposal submitted successfully",
            proposal,
        });
    } catch (error) {
        console.error("Error submitting teacher proposal:", error);
        if (req.files) {
            Object.values(req.files)
                .flat()
                .forEach((file) => {
...
const updateProposal = async (req, res) => {
    try {
        ...
        const { proposal_id: token_proposal_id, request_id: token_request_id } =
            req.updateData;
            ...
            if (
                ...
                if (existingPath) {
                    fs.unlink(existingPath, (err) => {
                        if (err)
                            console.error(
                                `Failed to delete old file: ${existingPath}`,
                                err,
                            );
                    });
                }
        ...
        proposal.pdf_url_part_A = updateFileField(
            "partA",
            proposal.pdf_url_part_A,
        );
        proposal.pdf_url_part_B = updateFileField(
            "partB",
        ...
        request.update_notes =
            updates.update_notes || "Proposal updated successfully";
        await request.save();
