...
const verifyReviewer = async (req, res) => {
  try {
    ...
    if (proposal_type === "student") {
      proposal = await StudentProposal.findById(req.proposal_id).select(
        "pdf_url_part_B fiscal_year"
      );
    } else if (proposal_type === "teacher") {
      proposal = await TeacherProposal.findById(req.proposal_id).select(
        "pdf_url_part_B fiscal_year"
      );
    } else {
      return res.status(401).json({ success: false, message: "Unauthorized" });
    ...
    if (!proposalDocument) {
      return res
        .status(404)
        .json({ success: false, message: "Proposal documents not found" });
    }
...
const addMark = async (req, res) => {
  try {
    ...
    if (!req.files || !req.files.marksheet) {
      return res
        .status(400)
        .json({ success: false, message: "Marksheet is required" });
    }

    // File Paths (Evaluation sheet and invoice are optional)
    const marksheetPath = path.join(
      __dirname,
      "..",
      "uploads",
      "marksheet",
    ...
    if (req.files.evaluation_sheet) {
      evaluationPath = path.join(
        ...
        "uploads",
        "evaluation_sheet",
        req.files.evaluation_sheet[0].filename
      );
    ...
    if (!assignment) {
      ...
      return res
        .status(404)
        .json({ success: false, message: "Reviewer assignment not found" });
    }
    ...
    if (invoiceUrl) {
      ...
      if (existingInvoice) {
        ...
        const previousInvoicePath = path.join(
          __dirname,
          "..",
      ...
      try {
        ...
        return res
          .status(500)
          .json({ success: false, message: "Failed to save invoice" });
      }
    }
    ...
    if (req.files) {
      ...
      if (
        req.files.evaluation_sheet &&
        fs.existsSync(req.files.evaluation_sheet[0].path)
      ) {
...
const submitInvoice = async (req, res) => {
  try {
    ...
    if (!invoice) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Not a valid invoice!" });
    } else {
    ...
    res
      .status(200)
      .json({ success: true, message: "Invoice submitted successfully!" });
  } catch (error) {
